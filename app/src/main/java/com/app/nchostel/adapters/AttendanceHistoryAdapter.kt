...
class AttendanceHistoryAdapter(private val context: Activity, private val mList: List<String>, private val user_id_i: String) : RecyclerView.Adapter<AttendanceHistoryAdapter.ViewHolder>() {
        ...
        val ItemsViewModel = mList[position]
         holder.title_tv!!.text = ItemsViewModel.replace("_"," ")
          holder.itemView.setOnClickListener {
            val intent = Intent(context, AttendanceHistoryDetailsActivity::class.java)
             intent.putExtra("month_key",ItemsViewModel)
             intent.putExtra("user_id",user_id_i)
