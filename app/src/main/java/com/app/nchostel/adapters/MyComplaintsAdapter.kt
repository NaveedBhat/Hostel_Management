...
class MyComplaintsAdapter(private val context: Activity, private val mList: List<ComplaintsModel>) : RecyclerView.Adapter<MyComplaintsAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {

        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.my_complaints_row, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.view_more_btn.setOnClickListener {
    ...
    class ViewHolder(ItemView: View) : RecyclerView.ViewHolder(ItemView) {
        ...
        val view_more_btn: AppCompatButton = itemView.findViewById(R.id.view_more_btn)
    }
    ...
    private fun deleteUser(
        ...
        val mDatabaseRef = database.reference
            .child(AppUtilObject.COMPLAINTS_TABLE_KEY)
            .child(mList.get(position).user_id).child(mList.get(position).id)
        mDatabaseRef.removeValue()
        viewHolder.itemView.setVisibility(View.GONE)
        Toast.makeText(context, "${mList.get(position).title} complaint has been deleted!", Toast.LENGTH_SHORT).show()
